...
function createMainWindow()
{
  ...
  mainWindow = new BrowserWindow(
  {
    ...
    show: false,
    title: 'MVG Control Panel',
  });
  // Register WindowID
  appConfig.set('mainWindowID', parseInt(mainWindow.id));
  appConfig.set('appUserModelId', 'MVG-Control-Panel');
  // Track window state
...
{
  // Default Logo
  const logoPath = path.resolve(__dirname, './static/images/logo.png');
  const logoData = fs.readFileSync(logoPath);
  const logoBase64String =
    'data:image/png+xml;base64,' + logoData.toString('base64');
  // Default Options
  const defaultOptions =
  {
    ...
    profile:
    {
      ...
      email: 'info@travelmvg.co.za',
      phone: '+27 (011) 000 0000',
      website: 'http://www.travelmvg.co.za/',
    },
    general:
    {
      language: 'en',
...
function initialize()
{
  app.setAppUserModelId('MVG-Control-Panel');
  app.on('ready', () => {
    app.setAppUserModelId('MVG-Control-Panel');
    createTourWindow();
    createMainWindow();
    ...
    // Show Window
    const { showWindow } = require('./helpers/tour');
    showWindow('startup');
  });
  // Reactivate the app
  app.on('activate', () =>
